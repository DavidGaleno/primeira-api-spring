...
public interface ConsultaRepository extends JpaRepository<Consulta, Long> {
    @Query(nativeQuery = true, value = "SELECT COUNT(c) FROM consultas c WHERE c.paciente_id = :idPaciente AND :dataInicio >= c.data_inicio AND :dataInicio < c.data_fim OR :intervaloAntesInicio > (c.data_inicio - INTERVAL '2 HOUR')")
    int findAppointmentsWithDateTimeForPacient(Long idPaciente,LocalDateTime dataInicio, LocalDateTime intervaloAntesInicio);

    @Query(nativeQuery = true, value = "SELECT COUNT(c) FROM consultas c WHERE c.medico_id = :idMedico AND :dataInicio >= c.data_inicio AND :dataInicio < c.data_fim OR :intervaloAntesInicio > (c.data_inicio - INTERVAL '2 HOUR')")
